  ...
  <el-card class="memo" :body-style="{ padding: '0px' }">
    ...
    <div class="content" style="margin: 10px">
      <div>
        ...
        <el-checkbox-group
          v-model="checkedmemo"
          @change="handleCheckedmemoChange"
        >
        <div>
          <el-checkbox
            v-for="(item, index) of sortmemo"
            :key="index"
            :label="item"
          >
              <div class="item">
                {{ item.msg }}まで、あと<span
                  ...
                  style="color: red"
                  >{{ item.count }}</span
                ><span v-else style="color: black">{{ item.count }}</span
                >日
              </div>
          </el-checkbox>
        </div>
    ...
    adddate: function () {
      ...
      if (this.value2 === "" || this.input === "" || this.issame()) {
        ...
        if (this.memo.length <= max_num) {
          let now = new Date();
          now = now.getTime();
          let count = Math.floor((this.value2 - now) / 86400000);
          this.data = {
            msg: this.input,
            count: count,
            date: this.value2,
